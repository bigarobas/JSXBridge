...
(function () {
    ...
    function getFolderContentFromPath(path,mask) {
        var folder = new Folder(path);
        return getFolderContent(folder,mask);
    }

    function getFolderContent(folder,mask) {
		var content = folder.getFiles(mask);
        return(content);
    }
    ...
    function getFolderOrCreate(folderPath) {
        var folder = new Folder(folderPath);
        if(!folder.exists) folder.create();
        return folder;
    }

    function getFileOrCreate(filePath) {
        var file = new File(filePath);
        var folder = getFolderOrCreate(file.path);
        return file;
    }
    ...
    return ( 
        {
            ...
            s2t : s2t,
            t2s : t2s,
            getFileOrCreate : getFileOrCreate,
            getFolderOrCreate : getFolderOrCreate,
